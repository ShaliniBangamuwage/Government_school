  ...
  return (
    <main className="space-y-6">
      <section className="overflow-hidden rounded-3xl border border-cyan-400/30 bg-gradient-to-br from-cyan-500/20 via-slate-900 to-slate-950 p-6 shadow-xl shadow-cyan-950/20 sm:p-8">
        <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div className="max-w-2xl">
            <p className="text-xs font-bold uppercase tracking-[0.28em] text-cyan-300">{t('interactiveLearning')}</p>
            <h1 className="mt-3 text-3xl font-black tracking-tight text-white sm:text-4xl">{t('simulators')}</h1>
            <p className="mt-3 max-w-xl text-base leading-7 text-slate-300">{t('simulatorDescription')}</p>
          </div>
      ...
      <section className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-bold text-white">{t('simulators')}</h2>
          <p className="mt-1 text-sm text-slate-400">{items.length} {items.length === 1 ? t('simulatorAvailable') : t('simulatorsAvailable')}</p>
        </div>
        <label className="relative block w-full sm:max-w-xs">
          <span className="sr-only">{t('searchSimulators')}</span>
          <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">⌕</span>
          <input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder={t('simulators')} className="w-full rounded-xl border border-slate-700 bg-slate-900 py-3 pl-9 pr-4 text-sm text-white outline-none transition placeholder:text-slate-500 focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/20" />
        </label>
      </section>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-slate-700 bg-slate-900/60 p-10 text-center">
          ...
          <h2 className="mt-4 text-lg font-bold text-white">{search ? t('noMatchingSimulators') : t('noSimulators')}</h2>
          <p className="mt-2 text-sm text-slate-400">{search ? t('tryDifferentSearch') : t('publishedActivities')}</p>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {visibleItems.map((item, index) => (
            <article key={item.id} className="group flex min-h-56 flex-col rounded-2xl border border-slate-800 bg-slate-900 p-5 shadow-lg shadow-slate-950/20 transition hover:-translate-y-1 hover:border-cyan-400/50 hover:bg-slate-800/90">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className="rounded-full bg-emerald-400/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-emerald-300">{t('readyToUse')}</span>
              </div>
              <h3 className="mt-5 text-xl font-bold text-white">{item.title ?? t('untitledSimulator')}</h3>
              <p className="mt-2 line-clamp-3 flex-1 text-sm leading-6 text-slate-400">{item.description ?? t('interactiveActivity')}</p>
              <button type="button" onClick={() => window.open(`/simulators/${item.id}`, '_blank', 'noopener,noreferrer')} className="mt-5 inline-flex w-full items-center justify-center rounded-xl bg-cyan-400 px-4 py-3 text-sm font-bold text-slate-950 transition hover:bg-cyan-300 focus:outline-none focus:ring-2 focus:ring-cyan-300 focus:ring-offset-2 focus:ring-offset-slate-900">
                {t('openWorkspace')} <span className="ml-2 text-base">↗</span>
              </button>
            </article>
          ))}
        </div>
