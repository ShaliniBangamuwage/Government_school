        ...
        <div className="mx-auto max-w-6xl space-y-6">
          <header className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-bold">{t('progressTitle')}</h1>
              <p className="mt-2 text-slate-300">{t('progressDescription')}</p>
            </div>
            <div className="flex gap-3 text-sm">
              <Link href="/student/mathematics/quizzes" className="rounded-xl bg-cyan-500 px-4 py-2 font-semibold text-slate-950">{t('takeQuiz')}</Link>
              <button type="button" onClick={() => void loadProgress()} className="rounded-xl border border-slate-700 px-4 py-2 text-slate-200">{t('refresh')}</button>
            </div>
          </header>

          {error ? <div className="rounded-xl border border-red-600 bg-red-500/10 p-4 text-red-200">{error}</div> : null}
          {loading ? <div className="rounded-2xl border border-slate-800 bg-slate-900 p-8 text-slate-300">{t('analyzingResults')}</div> : null}

          {!loading ? <>
            <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              {[
                [t('averageScore'), `${data.averagePercentage}%`, t('acrossSubmittedQuizzes')],
                [t('completed'), String(data.totalAttempts), t('submittedAttempts')],
                [t('passRate'), `${data.passRate}%`, `${data.passedCount} ${t('passedCount')}`],
                [t('bestScore'), `${data.bestScore}%`, t('personalBest')],
            ...
            <section className="rounded-2xl border border-cyan-500/30 bg-cyan-500/10 p-6">
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-300">{t('aiStudyCoach')}</p>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
                <h2 className="text-xl font-semibold">{t('topicPerformance')}</h2>
                {data.topicPerformance.length === 0 ? <p className="mt-4 text-slate-400">{t('topicInsights')}</p> : <div className="mt-5 space-y-5">{data.topicPerformance.map((topic) => <div key={topic.topic}><div className="mb-2 flex justify-between gap-4 text-sm"><span className="truncate text-slate-200">{topic.topic}</span><span className="font-semibold text-cyan-300">{topic.averagePercentage}%</span></div><div className="h-2 rounded-full bg-slate-700"><div className="h-2 rounded-full bg-cyan-400" style={{ width: `${Math.max(2, Math.min(100, topic.averagePercentage))}%` }} /></div><p className="mt-1 text-xs text-slate-500">{topic.attempts} {topic.attempts === 1 ? t('attempt') : t('attempts')}</p></div>)}</div>}
              </section>
              <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
                <h2 className="text-xl font-semibold">{t('recentResults')}</h2>
                {data.recentAttempts.length === 0 ? <p className="mt-4 text-slate-400">{t('noQuizResults')}</p> : <div className="mt-4 divide-y divide-slate-800">{data.recentAttempts.map((attempt) => <div key={attempt.id} className="flex items-center justify-between gap-4 py-4"><div className="min-w-0"><p className="truncate font-medium text-white">{attempt.quizTitle}</p><p className="mt-1 text-xs text-slate-500">{attempt.submittedAt ? new Date(attempt.submittedAt).toLocaleDateString() : t('recently')} · {attempt.score}/{attempt.totalQuestions}</p></div><span className={attempt.passed ? 'font-semibold text-emerald-400' : 'font-semibold text-amber-300'}>{Math.round(attempt.percentage)}%</span></div>)}</div>}
              </section>
            </div>
          </> : null}
        </div>
