...
export default function StudentMathematicsQuizzesPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setError(caughtError instanceof Error ? caughtError.message : t('loadingQuizzes'));
        setQuizzes([]);
      } finally {
        setLoading(false);
      }
  ...
  const handleStartQuiz = async (quizId: string) => {
    try {
      ...
      if (!attemptId) {
        throw new Error(t('quizAttemptMissing'));
      }

      router.push(`/student/mathematics/quizzes/${quizId}?attemptId=${encodeURIComponent(attemptId)}`);
    } catch (caughtError) {
      setError(caughtError instanceof Error ? caughtError.message : t('quizAttemptMissing'));
    } finally {
    ...
    <ProtectedRoute allowedRoles={['student']}>
      <main className="min-h-screen bg-slate-950 p-6 text-slate-50">
        <div className="mx-auto max-w-5xl space-y-6">
          <header>
            <p className="text-xs font-semibold uppercase tracking-[0.28em] text-cyan-400">{t('quizzes')}</p>
            <h1 className="mt-2 text-3xl font-bold">{t('publishedQuizzes')}</h1>
          </header>

          {error ? <div className="rounded-xl border border-red-600 bg-red-500/10 p-4 text-red-200">{error}</div> : null}
          {loading ? <div className="rounded-2xl border border-slate-800 bg-slate-900 p-8 text-slate-300">{t('loadingQuizzes')}</div> : null}

          {!loading && !error && quizzes.length === 0 ? (
            <div className="rounded-2xl border border-slate-800 bg-slate-900 p-8 text-slate-300">{t('noQuizzes')}</div>
          ) : null}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {quizzes.map((quiz) => (
                <article key={quiz.id} className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
                  <p className="text-xs uppercase tracking-[0.22em] text-cyan-300">{t('grade')} {quiz.grade} · {quiz.medium}</p>
                  <h2 className="mt-3 text-2xl font-semibold text-white">{quiz.title}</h2>
                  <div className="mt-4 space-y-2 text-sm text-slate-300">
                    <div>{t('questionCount')}: {quiz.questionCount}</div>
                    <div>{t('medium')}: {quiz.medium}</div>
                    <div>{t('published')}: {quiz.publishedAt ? new Date(quiz.publishedAt).toLocaleDateString() : t('recently')}</div>
                  </div>
                  <button
                    type="button"
                  ...
                  >
                    {startingQuizId === quiz.id ? t('starting') : t('startQuiz')}
                  </button>
                </article>
              ))}
            </div>
