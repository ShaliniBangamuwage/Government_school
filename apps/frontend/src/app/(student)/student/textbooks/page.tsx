...
export default function StudentTextbooksPage() {
  ...
  const activeSubjectLabel = subjectId === 'all' ? 'Mathematics' : subjects.find((subject) => subject.id === subjectId)?.name ?? 'Mathematics';
      ...
      <main className="min-h-screen bg-slate-950 p-8 text-slate-50">
        <div className="mx-auto max-w-6xl space-y-6">
          <header className="flex items-end justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.28em] text-cyan-400">My resources</p>
          ...
          <div className="grid gap-4 rounded-2xl border border-slate-800 bg-slate-900 p-4 md:grid-cols-4">
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search textbook" className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-white md:col-span-2" />
            {subjects.length > 1 ? (
            ...
            <button type="button" onClick={clearFilters} className="rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm font-medium text-white">Clear filters</button>
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900 px-4 py-3 text-sm text-slate-300">
            Showing: <span className="font-semibold text-white">{activeSubjectLabel}</span>
          </div>
          ...
          {!error && loading ? <div className="rounded-xl border border-slate-800 bg-slate-900 p-6 text-slate-300">Loading textbooks…</div> : null}

          {!error && !loading && filtered.length === 0 ? (
            <div className="rounded-xl border border-slate-800 bg-slate-900 p-6 text-slate-300">No verified textbooks match the current filters.</div>
          ) : null}
              ...
              {filtered.map((textbook) => (
                <article key={textbook.id} className="rounded-2xl border border-slate-800 bg-slate-900 p-5">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="rounded-full bg-emerald-500/15 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-300">Verified</span>
                  </div>
                  ...
                  {textbook.lastVerifiedAt ? <p className="mt-2 text-xs text-slate-400">Last verified: {new Date(String(textbook.lastVerifiedAt)).toLocaleDateString()}</p> : null}
                  <div className="mt-5 flex gap-3">
                    ...
                    {textbook.officialFileUrl ? (
                      <a href={`/api/textbooks/${textbook.id}/download`} rel="noreferrer" className="rounded-xl bg-cyan-500 px-3 py-2 text-sm font-semibold text-slate-950">Download through EduNexa</a>
                    ) : (
